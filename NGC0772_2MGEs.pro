...
;		stop
	    ...
	    print,mbh1*ml1,ml1,total(vrms-rmsmodel)^2/total(erms^2),mbh1
;		inc: 50.0; chi^2/DOF: 8.72; M/L: 0.465; M_BH: 9.28e+06; beta: 0.00
;		stop

		readcol,'HST_MGE/NGC0772_MGE_HST160_2mH.txt',mge_lum,mge_sigma,mge_q
...
;		mbh= 1e5
		...
		mbh=10d^float(lmbh    )
	    jam_axisymmetric_rms, $
	        mge_lum,mge_sigma,mge_q, mge_lum,mge_sigma,mge_q, $
			...
			if chi*1.001 lt total(vrms-rmsmodel)^2/total(erms^2) then break
		...
		al_legend,['2mass_k MGE, bh='+strn(alog10(mbh1*ml1),format='(G5.2)')+' log(Msun)','HST_160 bh='+strn(alog10(mbh2*ml2),format='(G5.2)')+' log(Msun)'],/right,psym=[1,4]
    WRITE_PNG, 'NGC0772_jeans_2MGEs.png', TVRD(/TRUE)
	    stop
	end
